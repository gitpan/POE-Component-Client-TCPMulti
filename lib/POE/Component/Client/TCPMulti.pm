...
sub create {
    ...
    $Code = {
        ...
        -incoming  => sub {
            my ($connection) = $_[ARG1];

            push @_, $Heap{$connection};
...
sub connect {
    ...
    $Heap{$id} = {
        ...
        -SERVER     => $server,
        -STAMP      => time,
        -ALARM      => $poe_kernel->delay_add
            ( -timeout => $Options{Timeout}, $server->ID ),
    };
    
    if ($Heap{$id}{-TIMEOUT}) {
        $Heap{$id}{-ALARM}  = $poe_kernel->alarm_set
            ( -timeout => time + $Heap{$id}{-TIMEOUT}, $id);
    }
    else {
        $Heap{$id}{-ALARM} = 0;
    }
